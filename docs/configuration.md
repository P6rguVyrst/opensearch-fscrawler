...
| `remove_deleted` | bool | `false` | Remove documents from the index when source files are deleted. Enable this only if you want the index to mirror the filesystem exactly. |
...
| `checksum` | string | `"sha256"` | Hash algorithm for file checksums (`MD5`, `SHA-1`, `SHA-256`, etc.). Always computed and stored in `file.checksum`. |
...
| `keep_history` | bool | `false` | Archive superseded document versions to a history index (`{name}_docs_history`) before overwriting. |
...
| `FSCRAWLER_FS_KEEP_HISTORY` | `fs.keep_history` | `true` / `false`. |
